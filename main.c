...
static void load_courses_from_csv(const char *path, Curso *cursos, size_t *count) {
...
int main(int argc, char *argv[]) {
    ...
    load_courses_from_csv(argv[1], cursos, &count);

    printf("Cursos cargados: %zu\n", count);
    printf("Primer curso: %s - %s\n", cursos[0].codigo, cursos[0].nombre);

    for (int i = 0; i < (int)count && i < 5; i++) {
        printf("Curso %d: %s | grupos:", i + 1, cursos[i].codigo);
        for (int j = 0; j < cursos[i].ngrupos; j++) {
            printf(" %d", cursos[i].grupos[j].numero);
        }
        printf("\n");
    }
